...
func (s *Service) recordImageFailure(token string, err error) {
	...
	if errors.Is(err, openaiweb.ErrContentPolicy) || openaiweb.IsImageReferenceRequired(err) || (errors.Is(err, openaiweb.ErrImageAssistantText) && !openaiweb.IsSkippedMainlineImageError(err) && !openaiweb.IsImageRequestEchoRetryError(err)) {
		return
	}
