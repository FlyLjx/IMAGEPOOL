...
Adobe 接入默认关闭，现有 `gpt-image-2` 仍走 ChatGPT Web；启用后新增的 72 个精确 `firefly-*` 图片模型走 Adobe。公开模型目录只返回生图模型，不提供 Chat Completions、Responses 或 Messages 接口。Adobe 模型名后缀固定分辨率与宽高比，调用时只需传 `model` 和 `prompt`。线路池支持本机公网出口直连和 HTTP(S)/SOCKS5 代理。Adobe 管理页可批量导入原版 `adobe2api` 的 `tokens.json` 和 Cookie/Profile JSON，并支持单账号、勾选批量、全部及默认 15 小时定时刷新 Token。生成时按最近使用时间直接轮询可用账号，不建立 ARP 或生成租约。启用前必须使用 PostgreSQL，在 `.env` 设置随机的 `IMAGE_POOL_MASTER_KEY`，再将 `configs/config.json` 的 `adobe.enabled` 改为 `true`。完整配置和导入格式见 [docs/adobe-integration.md](docs/adobe-integration.md)。
