...
- 图片 SSE 的响应头、无字节空闲窗口均为 60 秒；同一次已提交生图从流到会话轮询共用 180 秒预算。轮询从 3 秒开始并退避到 10 秒。
...
- 普通 `gpt-image-2` 请求使用官网 Web 的 `auto` model；`image_web_model_slug` 保留为兼容配置，不参与当前普通生图路由。
