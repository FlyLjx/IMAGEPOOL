...
4. `POST /backend-api/f/conversation/prepare`：准备图片会话，关键字段：
   - `action: "next"`
   - `model: config.image_web_model_slug`，默认 `gpt-5-5`
...
- `no available free image quota`：直接从号池删除并切换下一个账号。
...
- `image_web_model_slug` 控制图片会话 prepare 中的 Web model slug，默认 `gpt-5-5`。
