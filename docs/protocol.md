...
- Adobe 模型只接受图片生成、图片编辑和异步图片任务接口；基础模型名固定 1K/2K/4K 分辨率，客户端 `aspect_ratio` 匹配隐藏精确变体。缺少 `aspect_ratio` 时从 `size=宽x高` 推导比例，`size` 不覆盖固定分辨率或 Adobe 最终像素；如果计算出的比例不在当前模型支持列表中，则选择最近的后台比例，无效或缺失参数才回退同分辨率 `1:1`。GPT Image 1K/2K 固定快速档，4K 默认标准档，仅显式高质量参数走高质量档。
